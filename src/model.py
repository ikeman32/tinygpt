...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        # Weight tying: share output projection weights with token embeddings
        wte = cast(nn.Embedding, self.transformer["wte"])
        wte.weight = self.lm_head.weight

    def forward(self, idx: torch.Tensor, targets: torch.Tensor | None = None, attention_mask: torch.Tensor | None = None):
        B, T = idx.shape
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # PyTorch SDPA causal attention with key padding mask
